...
public class CRLCache
{
  ...
  public CRL getCRLFromURL (@Nullable final String sCRLURL)
  {
    ...
    if (aStaleCRL != null)
    {
      LOGGER.warn ("The re-fetch for CRL URL '" + sCRLURL + "' was unsuccessful, so keeping the previous CRL version.");
   ...
   * {@link CRL} verbatim. Any in-process caller that obtains a reference to this cache can use this
   * method (or the inherited {@code putInCache(...)} overloads) to plant an arbitrary {@link CRL} -
   * including one with an empty revoked-cert list - and thereby subvert CRL-based revocation
   * checking for the affected URL. The PKIX validator only enforces the cached CRL's
