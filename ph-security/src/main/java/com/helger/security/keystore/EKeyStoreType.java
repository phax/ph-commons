...
public enum EKeyStoreType implements IKeyStoreType
{
  ...
  PKCS11 ("PKCS11", false),
  /**
   * The PKCS12 key store type is slightly better than JKS and is supported only in Java 1.9. In
   * Java 1.8 and earlier this is only supported when BouncyCastle is contained.
   */
  PKCS12 ("PKCS12", true),
  /**
   * The first "BKS" is a keystore that will work with the keytool in the same fashion as the Sun
   * "JKS" keystore. The keystore is resistant to tampering but not inspection.<br>
  ...
  BKS ("BKS", true),
  /**
   * The BCFKS key store is designed to be FIPS compliant. It is available in approved-mode
   * operation and is also capable of storing some secret key types in addition to public/private
   * keys and certificates. The BCFKS key store uses PBKDF2 with HMAC SHA512 for password to key
   * conversion and AES CCM for encryption. Passwords are encoded for conversion into keys using
