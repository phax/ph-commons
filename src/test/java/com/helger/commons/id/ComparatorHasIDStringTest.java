...
public final class ComparatorHasIDStringTest
{
  ...
  public void testAll ()
  {
    ...
    CollectionHelper.getSortedInline (aList, new ComparatorHasIDString <IHasID <String>> (ESortOrder.ASCENDING));
    assertEquals ("3", aList.get (0).getID ());
    assertEquals ("5", aList.get (1).getID ());
    assertEquals ("7", aList.get (2).getID ());

    CollectionHelper.getSortedInline (aList, new ComparatorHasIDString <IHasID <String>> (ESortOrder.DESCENDING));
