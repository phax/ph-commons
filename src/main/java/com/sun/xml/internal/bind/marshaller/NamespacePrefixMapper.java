 ...
 * holder.
 *
 * Plutext Pty Ltd elects to include this software in this distribution under
 * the CDDL license.
 *
 * This file was obtained from JAXB RI 2.2.4.
 * Plutext Pty Ltd modified the file by changing the package from
 * com.sun.xml.bind.marshaller to com.sun.xml.internal.bind.marshaller
 */
...
public abstract class NamespacePrefixMapper
{
   ...
   * intended to be overrided by a derived class.
   * <p>
   ...
   * </p>
   * <ol>
   * <li>If the prefix returned is already in use as one of the in-scope
   * namespace bindings. This is partly necessary for correctness (so that we
   * don't unexpectedly change the meaning of QNames bound to {@link String}),
   * partly to simplify the marshaller.</li>
   * <li>If the prefix returned is "" yet the current {@link JAXBContext}
   ...
   *
   * @param namespaceUri
   *        The namespace URI for which the prefix needs to be found. Never be
   *        null. "" is used to denote the default namespace.
   * @param suggestion
   *        When the content tree has a suggestion for the prefix to the given
   ...
   * @param requirePrefix
   *        If this method is expected to return non-empty prefix. When this
   ...
   *         case, the system will generate a prefix for you. Otherwise the
   *         system will try to use the returned prefix, but generally there's
   *         no guarantee if the prefix will be actually used or not. return ""
   *         to map this namespace URI to the default namespace. Again, there's
   *         no guarantee that this preference will be honored. If this method
   *         returns "" when requirePrefix=true, the return value will be
   *         ignored and the system will generate one.
   * @since JAXB 1.0.1
   */
   ...
   * </p>
   * <p>
   * If you know in advance that you are going to use a certain set of namespace
   * URIs, you can override this method and have the marshaller declare those
   ...
   *         string to represent the empty namespace URI and the default prefix.
   *         Null is not allowed as a value in the array.
   ...
   * JAXB RI.)
   * <p>
   * Sometimes JAXB is used to marshal an XML document, which will be used as a
   * subtree of a bigger document. When this happens, it's nice for a JAXB
   ...
   * {@link org.w3c.dom.Node}, because those output format allows us to inspect
   * what's currently available as in-scope namespace binding. However, with
   * other output format, such as {@link java.io.OutputStream}, the JAXB RI
   * cannot do this automatically. That's when this method comes into play.
   * </p>
   * <p>
   * Namespace bindings returned by this method will be used by the JAXB RI, but
   * will not be re-declared. They are assumed to be available when you insert
