...
public enum EXMLParserFeature implements IHasName
{
  ...
  * URIs. By default, the two will simply be concatenated, but the namespace-sep
  * core property allows the application to specify a delimiter string for
  * separating the URI part and the local part.<br>
 ...
 /**
  * When true: The methods of the org.xml.sax.ext.EntityResolver2 interface will
  * be used when an object implementing this interface is registered with the
  * parser using setEntityResolver.<br>
  ...
  * specified.<br>
  * When false: Validation is determined by the state of the {@link #VALIDATION}
  * feature.<br>
  * Default: false<br>
  ...
  * checking are controlled by this option.<br>
  ...
  * When false: Only the first schema location hint encountered by the processor
  * will be used to locate the components for a given target namespace.<br>
  * Default: false<br>
 ...
 /**
  * When true: Notifies the handler of built-in entity boundaries (e.g &amp;) in
  * the document via the start/endEntity callbacks.<br>
  ...
  * using invalid URI characters (space, for example)<br>
  * Default: false<br>
 ...
 /**
  * When true: Create EntityReference nodes in the DOM tree. The EntityReference
  * nodes and their child nodes will be read-only.<br>
  ...
  * fully expanded substitution text will be created.<br>
  * Default: true<br>
 ...
 /**
  * When true: All element names, prefixes, attribute names, namespace URIs, and
  ...
  * <br>
  * When false: The document specified standalone="no" in its XML declaration or
  * the standalone document declaration was absent.<br>
  * Read-only!<br>
