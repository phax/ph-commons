...
public final class PDTWebDateHelper
{
   ...
   */
  @Nonnull
  public static IPair <String, ZoneId> extractDateTimeZone (@Nonnull final String sDate)
  {
    ...
    for (final PDTZoneID aSupp : PDTZoneID.getDefaultZoneIDs ())
    {
      ...
      if (sDate.endsWith (" " + sDTZ))
        return Pair.create (sDate.substring (0, nDateLen - (1 + sDTZ.length ())), aSupp.getZoneID ());
      if (sDate.endsWith (sDTZ))
        return Pair.create (sDate.substring (0, nDateLen - sDTZ.length ()), aSupp.getZoneID ());
    }
  ...
  public static ZonedDateTime getDateTimeFromRFC822 (@Nullable final String sDate)
  {
    ...
    final IPair <String, ZoneId> aPair = extractDateTimeZone (sDate.trim ());
    return parseZonedDateTimeUsingMask (RFC822_MASKS, aPair.getFirst (), aPair.getSecond ());
  }
