...
public final class HelpFormatterTest
{
  ...
  public void testPrintOptions () throws Exception
  {
    ...
    options = new Options ().addOption (Option.builder ("a").longOpt ("aaa").desc ("dddd dddd dddd dddd").build ());
    ...
    options = new Options ().addOption (Option.builder ("a").longOpt ("aaa").desc ("dddd dddd dddd dddd").build ())
                            .addOption (Option.builder ("b").desc ("feeee eeee eeee eeee").build ());
    expected = lpad +
               "-a,--aaa" +
               dpad +
               "dddd dddd" +
  ...
  public void testAutomaticUsage () throws Exception
  {
    ...
    options = new Options ().addOption (Option.builder ("a").desc ("aaaa aaaa aaaa aaaa aaaa").build ())
                            .addOption (Option.builder ("b").desc ("bbb").build ());
    hf.printUsage (pw, 60, "app", options);
    pw.flush ();
    assertEquals ("simple auto usage", expected, out.getAsString (StandardCharsets.ISO_8859_1).trim ());
    out.reset ();
  ...
  {
    final Options opts = new Options ();
    opts.addOption (Option.builder ("a").desc ("first").build ());
    opts.addOption (Option.builder ("b").desc ("second").build ());
    opts.addOption (Option.builder ("c").desc ("third").build ());
  ...
  {
    final Options opts = new Options ();
    opts.addOption (Option.builder ("c").desc ("first").build ());
    opts.addOption (Option.builder ("b").desc ("second").build ());
    opts.addOption (Option.builder ("a").desc ("third").build ());
  ...
  {
    final Option option = Option.builder ("f").required ().build ();
  ...
  {
    final Option option = Option.builder ("f").oneArg ().required (true).build ();
  ...
  public void testOptionWithoutShortFormat ()
  {
    ...
    options.addOption (Option.builder ().longOpt ("bbb").desc ("bbbbbbb").build ());
  ...
  {
    final Options options = new Options ();
    options.addOption (Option.builder ("f").oneArg ().desc ("the file").build ());
    options.addOption (Option.builder ("s").longOpt ("size").desc ("the size").oneArg ().argName ("SIZE").build ());
    options.addOption (Option.builder ().longOpt ("age").desc ("the age").oneArg ().build ());
