...
import com.helger.base.id.mock.MockHasLongID;
...
public final class IHasLongIDTest
{
  ...
  public void testAll ()
  {
    final List <? extends IHasLongID> aList = Arrays.asList (new MockHasLongID (5),
                                                             new MockHasLongID (3),
                                                             new MockHasLongID (7));
    aList.sort (IHasLongID.getComparatorID ());
    assertEquals (3, aList.get (0).getID ());
    assertEquals (5, aList.get (1).getID ());
    assertEquals (7, aList.get (2).getID ());

    aList.sort (IHasLongID.getComparatorID ().reversed ());
