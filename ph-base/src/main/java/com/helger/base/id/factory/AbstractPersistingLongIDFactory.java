...
/**
 * This implementation of {@link ILongIDFactory} reads IDs from a device. It does it by reserving a
 * range of <em>n</em> IDs so that not each ID reservation requires IO. If only 1 ID is effectively
 * used, the other <em>n</em>-1 IDs are lost and will never be assigned to any object again.
 *
...
public abstract class AbstractPersistingLongIDFactory implements ILongIDFactory
{
  ...
  {
    setReserveCount (nReserveCount);
  }

  /**
   * @return The number of IDs to reserve, as provided in the constructor. Always &gt; 0.
   */
  @Nonnegative
  ...
  protected final int internalGetReserveCountUnlocked ()
  {
    return m_nReserveCount;
  }

  /**
   * Dynamically set the reserve count of this factory. May be a performance improvement when this
   * is used in an area where many IDs are required.
   *
   * @param nReserveCount
   *        The reserve count to use. Must be &gt; 0.
   * @since 12.1.3
   */
  ...
  /**
   * Read the current ID from the device. In case the method is called for a non-initialized device,
   * 0 should be returned.<br>
   * The update should write the read value plus the passed reserve count back to the device. This
   * method should perform an atomic read and update to avoid that ID can be reused.<br>
   * Pseudo code:
   *
   * <pre>
   ...
   * @return 0 if this method is called for a non-initialized device, the value read from the device
   *         otherwise or {@link com.helger.base.CGlobal#ILLEGAL_ULONG} in case of an error.
   */
