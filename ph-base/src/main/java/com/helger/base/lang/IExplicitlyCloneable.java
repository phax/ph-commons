 ...
 * use a type argument for the result of "clone" because otherwise this interface cannot be used on
 * multiple levels of the inheritance tree. So implementing classes should just change the return
 * type accordingly.
 *
...
public interface IExplicitlyCloneable extends Cloneable
{
   ...
   * fields of the object returned by {@code super.clone} before returning it. Typically, this means
   * copying any mutable objects that comprise the internal "deep structure" of the object being
   * cloned and replacing the references to these objects with references to the copies. If a class
   * contains only primitive fields or references to immutable objects, then it is usually the case
   * that no fields in the object returned by {@code super.clone} need to be modified.
   * <p>
   ...
   *         if the object's class does not support the {@code Cloneable} interface. Subclasses that
   *         override the {@code clone} method can also throw this exception to indicate that an
