...
public class NonBlockingBufferedReader extends Reader
{
   ...
   *
   * @return The character read, as an integer in the range 0 to 65535 ( <code>0x00-0xffff</code>),
   ...
   *
   * @return A {@link String} containing the contents of the line, not including any
   *         line-termination characters, or <code>null</code> if the end of the stream has been
   *         reached
   * @exception IOException
   *            If an I/O error occurs
   */
   ...
   *        Limit on the number of characters that may be read while still preserving the mark. An
   *        attempt to reset the stream after reading characters up to this limit or beyond may
   *        fail. A limit value larger than the size of the input buffer will cause a new buffer to
   *        be allocated whose size is no smaller than limit. Therefore large values should be used
   *        with care.
   * @exception IllegalArgumentException
   ...
   * {@code BufferedReader}. The {@link Stream} is lazily populated, i.e., read only occurs during
   * the <a href="../util/stream/package-summary.html#StreamOps">terminal stream operation</a>.
   * <p>
   ...
   *
   * @return a {@code Stream<String>} providing the lines of text described by this
   *         {@code BufferedReader}
   * @since 1.8
   */
  @NonNull
