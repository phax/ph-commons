...
{
  /**
   * Get the input stream to read from the object. Each time this method is called, a new
   * {@link InputStream} needs to be created.
   *
  ...
  /**
   * Get a buffered input stream to read from the object. Each time this method is called, a new
   * {@link InputStream} needs to be created. Internally invokes {@link #getInputStream()}.
   *
   * @return <code>null</code> if resolving failed.
   * @since 9.1.8
   */
  ...
  /**
   * Perform something with the {@link InputStream} of this object and making sure, that it gets
   * closed correctly afterwards.
   *
   * @param <T>
   *        The result type of handling the {@link InputStream}
   * @param aFunc
   *        The function to be invoked to read from the {@link InputStream}. This function needs to
   *        be able to deal with a <code>null</code>-parameter.
  ...
  /**
   * Perform something with the buffered {@link InputStream} of this object and making sure, that it
   * gets closed correctly afterwards.
   *
   * @param <T>
   *        The result type of handling the buffered {@link InputStream}
   * @param aFunc
   *        The function to be invoked to read from the buffered {@link InputStream}. This function
   *        needs to be able to deal with a <code>null</code>-parameter.
   * @return The result of the function. May be <code>null</code>.
   * @throws IOException
   *         In case reading from the InputStream fails
   * @since 11.1.5
   ...
   *
   * @return <code>true</code> if the input stream can be acquired more than once,
